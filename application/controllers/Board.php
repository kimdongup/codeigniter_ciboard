...
class Board extends CI_Controller {
	...
	public function lists()
	{
		...
		//페이지네이션 설정
		$config['base_url'] = '/ciboard/index.php/board/lists/ci_board'.$page_url.'/page/'; //페이징 주소
	...
	function write()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			if ( $this->form_validation->run() == TRUE )
			{
				...
				{
					//글 작성 성공시 게시판 목록으로
					alert('입력되었습니다.', '/ciboard/index.php/board/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				else
				{
					//글 실패시 게시판 목록으로
					alert('다시 입력해 주세요.', '/ciboard/index.php/board/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
		...
		{
			alert('로그인후 작성하세요', '/ciboard/index.php/auth/login/');
			exit;
		}
 	}
	...
	function modify()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			{
				alert('본인이 작성한 글이 아닙니다.', '/ciboard/index.php/board/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$pages);
				exit;
			}
			...
			if ( $this->form_validation->run() == TRUE )
			{
				if ( !$this->input->post('subject', TRUE) AND !$this->input->post('contents', TRUE) )
				{
					...
					alert('비정상적인 접근입니다.', '/ciboard/index.php/board/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				...
				{
					//글 작성 성공시 게시판 목록으로
					alert('수정되었습니다.', '/ciboard/index.php/board/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				else
				{
					//글 수정 실패시 글 내용으로
					alert('다시 수정해 주세요.', '/ciboard/index.php/board/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$pages);
					exit;
				}
		...
		{
			alert('로그인후 수정하세요', '/ciboard/index.php/auth/login/');
			exit;
		}
 	}
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/ciboard/index.php/board/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$this->uri->segment(7));
				exit;
			}
			...
			{
				//삭제가 성공한 경우
				alert('삭제되었습니다.', '/ciboard/index.php/board/lists/'.$this->uri->segment(3).'/page/'.$this->uri->segment(7));
			}
			else
			{
				//삭제가 실패한 경
				alert('삭제 실패하였습니다.', '/ciboard/index.php/board/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$this->uri->segment(7));
			}
		}
		else
		{
			alert('로그인후 삭제하세요', '/ciboard/index.php/auth/login/');
			exit;
		}
 	}
